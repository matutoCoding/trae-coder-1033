...
import { FileText, Download, Printer, Clock, MapPin, AlertTriangle, CheckCircle, TrendingUp, DollarSign, Users, Ship, RefreshCw, Zap } from 'lucide-react';
import { useStore } from '../../store/useStore';
import { eventStatusLabels, oilTypeLabels } from '../../types';
import type { EventSummaryReport } from '../../types';
import { formatDateTime } from '../../utils/helpers';

const EventSummary: React.FC = () => {
  const { currentEvent, generateSummaryReport, getEventSummaryReport, saveSummaryReport } = useStore();
  const [activeTab, setActiveTab] = useState<'overview' | 'resources' | 'ecology' | 'lessons'>('overview');
  const [report, setReport] = useState<EventSummaryReport | undefined>(() => {
    if (currentEvent) {
      return getEventSummaryReport(currentEvent.id);
    }
    return undefined;
  });
  ...
  useEffect(() => {
    if (currentEvent) {
      const saved = getEventSummaryReport(currentEvent.id);
      setReport(saved);
    } else {
      setReport(undefined);
    }
  }, [currentEvent?.id, getEventSummaryReport]);
    ...
    <div className="space-y-6">
      ...
      {report && (
        <div className="flex items-center gap-2 text-sm text-gray-500">
          <Clock size={14} />
          <span>报告生成时间：{report.generatedAt}</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 text-ocean-100 text-sm">
              <Clock size={14} />
              持续 {report?.duration || '计算中...'}
            </div>
          </div>
          <div className="text-right">
            <p className="text-ocean-100 text-sm">回收率</p>
            <p className="text-3xl font-bold">{report?.recoveryStats.recoveryRate ?? 0}%</p>
            <p className="text-ocean-100 text-sm mt-1">已回收 {report?.recoveryStats.totalCollected ?? 0} 吨</p>
          </div>
        </div>
      </div>
      ...
      {report ? (
        <>
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              { label: '已回收', value: report.recoveryStats.totalCollected, unit: '吨', icon: <TrendingUp size={24} />, color: 'bg-alert-green' },
              { label: '总体进度', value: report.overallProgress, unit: '%', icon: <CheckCircle size={24} />, color: 'bg-alert-blue' },
              { label: '参与人员', value: report.resourceInvestment.personnel, unit: '人', icon: <Users size={24} />, color: 'bg-ocean-700' },
            ].map((stat, idx) => (
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="p-4 bg-gray-50 rounded-lg">
                      ...
                      <p className="font-medium text-gray-800">{report.basicInfo.eventName}</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">发生地点</p>
                      <p className="font-medium text-gray-800">{report.basicInfo.location}</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">溢油类型</p>
                      <p className="font-medium text-gray-800">{report.basicInfo.oilType}</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">预估溢油量</p>
                      <p className="font-medium text-gray-800">{report.basicInfo.estimatedVolume} 吨</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">发生时间</p>
                      <p className="font-medium text-gray-800">{report.basicInfo.occurrenceTime}</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">接报人</p>
                      <p className="font-medium text-gray-800">{report.basicInfo.reporter}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="p-4 bg-ocean-50 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-2xl font-bold text-ocean-700">{report.overallProgress}%</span>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="p-4 bg-green-50 rounded-lg border border-green-100">
                      ...
                      <p className="text-xl font-bold text-green-700">{report.recoveryStats.byType.skimmer.toFixed(1)} 吨</p>
                    </div>
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                      <p className="text-sm text-blue-600 mb-1">消油剂处理</p>
                      <p className="text-xl font-bold text-blue-700">{report.recoveryStats.byType.dispersant.toFixed(1)} 吨</p>
                    </div>
                    <div className="p-4 bg-purple-50 rounded-lg border border-purple-100">
                      <p className="text-sm text-purple-600 mb-1">岸线清理</p>
                      <p className="text-xl font-bold text-purple-700">{report.recoveryStats.byType.shoreline.toFixed(1)} 吨</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-gray-700 leading-relaxed whitespace-pre-line">{report.disposalProcess}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-4">
                    <div className="p-4 bg-ocean-50 rounded-lg text-center">
                      ...
                      <p className="text-2xl font-bold text-ocean-700">{report.resourceInvestment.vessels} 艘</p>
                    </div>
                    <div className="p-4 bg-green-50 rounded-lg text-center">
                      <Zap size={32} className="mx-auto text-green-600 mb-2" />
                      <p className="text-sm text-gray-500">设备</p>
                      <p className="text-2xl font-bold text-green-700">{report.resourceInvestment.equipment} 台</p>
                    </div>
                    <div className="p-4 bg-blue-50 rounded-lg text-center">
                      <Users size={32} className="mx-auto text-blue-600 mb-2" />
                      <p className="text-sm text-gray-500">人员</p>
                      <p className="text-2xl font-bold text-blue-700">{report.resourceInvestment.personnel} 人</p>
                    </div>
                    <div className="p-4 bg-orange-50 rounded-lg text-center">
                      <AlertTriangle size={32} className="mx-auto text-orange-600 mb-2" />
                      <p className="text-sm text-gray-500">物资</p>
                      <p className="text-2xl font-bold text-orange-700">{report.resourceInvestment.materials} 类</p>
                    </div>
                    <div className="p-4 bg-purple-50 rounded-lg text-center">
                      <TrendingUp size={32} className="mx-auto text-purple-600 mb-2" />
                      <p className="text-sm text-gray-500">围油栏</p>
                      <p className="text-2xl font-bold text-purple-700">{report.resourceInvestment.containmentLength} 米</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      { icon: <Ship size={20} />, name: '清污船舶', value: `${report.resourceInvestment.vessels}艘`, color: 'bg-ocean-100 text-ocean-700' },
                      { icon: <Zap size={20} />, name: '作业设备', value: `${report.resourceInvestment.equipment}台`, color: 'bg-green-100 text-green-700' },
                      { icon: <Users size={20} />, name: '作业人员', value: `${report.resourceInvestment.personnel}人`, color: 'bg-blue-100 text-blue-700' },
                      { icon: <DollarSign size={20} />, name: '物资种类', value: `${report.resourceInvestment.materials}类`, color: 'bg-purple-100 text-purple-700' },
                      { icon: <TrendingUp size={20} />, name: '围油栏总长度', value: `${report.resourceInvestment.containmentLength}米`, color: 'bg-orange-100 text-orange-700' },
                    ].map((item, idx) => (
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-red-50 rounded-lg border border-red-100">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-medium text-alert-red">损害等级：{report.ecologyImpact.damageLevel}/10</span>
                      </div>
                      <p className="text-sm text-gray-600">影响面积约 {report.ecologyImpact.affectedArea} 平方公里</p>
                    </div>
                    <div>
                      <h5 className="font-medium text-gray-700 mb-2">敏感资源受影响</h5>
                      <div className="flex flex-wrap gap-2">
                        {report.ecologyImpact.sensitiveResources.length > 0 ? (
                          report.ecologyImpact.sensitiveResources.map((r, i) => (
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {report.ecologyImpact.affectedSpecies.length > 0 ? (
                          report.ecologyImpact.affectedSpecies.map((s, i) => (
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-xl font-bold text-ocean-700">{report.ecologyImpact.estimatedRecoveryTime}</p>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg h-72 overflow-y-auto">
                    <p className="text-sm text-gray-600 leading-relaxed">
                      {report.ecologyImpact.damageLevel > 0
                        ? `本次溢油事件对周边海域生态造成了一定程度的影响，损害等级为 ${report.ecologyImpact.damageLevel}/10 级。影响面积约 ${report.ecologyImpact.affectedArea} 平方公里，涉及 ${report.ecologyImpact.sensitiveResources.length} 类敏感资源和 ${report.ecologyImpact.affectedSpecies.length} 种受保护物种。预计恢复时间为 ${report.ecologyImpact.estimatedRecoveryTime}。`
            ...
            <div className="relative">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { time: report.generatedAt, title: '报告生成', desc: '生成事件总结报告', status: 'success' },
                ].map((event, idx) => (
        ...
        <div className="card text-center py-16">
          ...
          <p className="text-gray-400 mb-6">点击上方「生成报告草稿」按钮，一键生成事件总结报告</p>
          <button
            onClick={handleGenerateReport}
            disabled={isGenerating}
